...
# Batch compile + parallel run + result table for geo_descent.cpp.
#
# Usage:
#   chmod +x run_geo_descent_batch.sh
#   JOBS=16 SEEDS="1 2 3 4 5" TIME_LIMIT=3600 ./run_geo_descent_batch.sh
#
# Outputs:
...
JOBS="${JOBS:-}"
SEEDS="${SEEDS:-1}"
SRC="${SRC:-geo_descent.cpp}"
EXE="${EXE:-geo_descent}"
INSTANCE_DIR="${INSTANCE_DIR:-geo_instances}"
OUTDIR="${OUTDIR:-descent_batch_results_$(date +%Y%m%d_%H%M%S)}"
...
while IFS=$'\t' read -r inst seed log; do
    ...
    best_profit="$(awk '/Iterated descent done/ {for(i=1;i<=NF;i++) if($i ~ /^bestProfit=/){split($i,a,"="); print a[2]}}' "$log" | tail -1)"
    best_time="$(awk '/Iterated descent done/ {for(i=1;i<=NF;i++) if($i ~ /^bestTime=/){split($i,a,"="); print a[2]}}' "$log" | tail -1)"
